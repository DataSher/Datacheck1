...
def calculate_efficiency(inlet_pressure, inlet_temp, outlet_pressure, outlet_temp):
    """
    Calculate the efficiency of a steam cylinder based on inlet and outlet properties.

    Parameters:
    inlet_pressure (float): Inlet pressure in kg/cm² gauge.
    inlet_temp (float): Inlet temperature in Celsius.
    outlet_pressure (float): Outlet pressure in kg/cm² gauge.
    ...
    inlet_pressure_bar_abs = (inlet_pressure + 1.01325)  # Convert from kg/cm² gauge to bar absolute
    outlet_pressure_bar_abs = (outlet_pressure + 1.01325)  # Convert from kg/cm² gauge to bar absolute
...
def compute_section_efficiencies(df):
    """
    Compute efficiencies for each cylinder section based on input data.

    Parameters:
    df (pd.Series): A row of the DataFrame containing pressure and temperature data.

    Returns:
    dict: A dictionary of calculated efficiencies for each cylinder section.
    """
    efficiencies = {}
...
for index, row in df_filtered.iterrows():
    ...
    date_str = row['Date'].strftime('%m-%d-%Y')  # Convert date to MM-DD-YYYY format
    time_str = row['Time']  # Assuming Time is in either 'HH:MM' or 'HH:MM:SS' format

    # Convert the normalized time to 12-hour format with AM/PM
    time_24 = pd.to_datetime(time_str, format='%H:%M:%S', errors='coerce')
    if time_24 is None:
        raise ValueError(f"Invalid time format: {time_str}")
    
    time_12 = time_24.strftime('%I:%M:%S %p')  # Convert to 12-hour format with AM/PM

    # Combine Date and Time as string
    timestamp_str = f"{date_str} {time_12}"  # e.g., "09-02-2024 05:33:00 PM"
    
    # Convert to datetime with the correct format
    timestamp = pd.to_datetime(timestamp_str, format='%m-%d-%Y %I:%M:%S %p', dayfirst=True)  # Convert to datetime

    efficiencies['Timestamp'] = timestamp  # Add the timestamp for each row
...
efficiency_df['Timestamp'] = efficiency_df['Timestamp'].dt.strftime('%m-%d-%Y %H:%M:%S')
...
efficiency_df_last_three = efficiency_df.tail(3)
...
def plot_efficiency_vs_load():
    """
    Plot the efficiency vs load curve for actual and design efficiencies of HP, IP, and LP cylinders.
    """
    ...
    plt.plot(design_efficiency_df['Load'], design_efficiency_df['HP Cylinder'].apply(lambda x: float(x.strip('%'))), label='HP Cylinder Design', linestyle='--', marker='x')
    plt.plot(design_efficiency_df['Load'], design_efficiency_df['IP Cylinder'].apply(lambda x: float(x.strip('%'))), label='IP Cylinder Design', linestyle='--', marker='x')
    plt.plot(design_efficiency_df['Load'], design_efficiency_df['LP Cylinder till 3rd extraction'].apply(lambda x: float(x.strip('%'))), label='LP Cylinder Design', linestyle='--', marker='x')
...
# Function to calculate overall performance index
def calculate_overall_performance_index(efficiency_df, design_efficiency_df):
    # Merge design efficiencies into efficiency_df
    merged_df = efficiency_df.merge(
        design_efficiency_df[['Load', 'HP Cylinder', 'IP Cylinder', 'LP Cylinder till 3rd extraction']],
        on='Load', suffixes=('', '_Design'), how='left'
    )

    # Calculate OPI
    merged_df['Overall Performance Index'] = (
        (merged_df['HP Cylinder'].str.rstrip('%').astype(float) / merged_df['HP Cylinder_Design'].str.rstrip('%').astype(float)) *
         (merged_df['IP Cylinder'].str.rstrip('%').astype(float) / merged_df['IP Cylinder_Design'].str.rstrip('%').astype(float)) *
         (merged_df['LP Cylinder till 3rd extraction'].str.rstrip('%').astype(float)/merged_df['LP Cylinder till 3rd extraction_Design'].str.rstrip('%').astype(float)))*100

    return merged_df[['Timestamp', 'Load', 'HP Cylinder', 'IP Cylinder', 'LP Cylinder till 3rd extraction', 'Overall Performance Index']]

# Calculate overall performance index and add to efficiency DataFrame
efficiency_with_opi = calculate_overall_performance_index(efficiency_df, design_efficiency_df)

# Show only the last three rows with overall performance index
efficiency_with_opi_last_three = efficiency_with_opi.tail(3)

# Update Streamlit display section
with col8:
    st.subheader("Actual Turbine Efficiencies with Overall Performance Index")
    
    # Display the DataFrame with OPI
    st.dataframe(efficiency_with_opi_last_three)

    # Download option for efficiencies with OPI
    csv_efficiency_with_opi = efficiency_with_opi_last_three.to_csv(index=False).encode('utf-8')
    st.download_button("Download Efficiency Data with OPI", csv_efficiency_with_opi, "efficiency_with_opi_data.csv", "text/csv")
    
    # Add button to show efficiency vs load curve
